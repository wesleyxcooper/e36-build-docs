...
- [ ] 🔧 **Source AS79 and Maven HD30 crimpers before starting the bulkhead build**   [ELECTRICAL · TOOLING]
  - **AS79 firewall bulkhead (size-22 contacts):** requires **Daniels DMC AFM8** (M22520/2-01, $601.65 — [dmctools.com](https://dmctools.com/afm8)) + **K42 positioner** (pin contacts, $112.64 — [deltaintl.com](https://deltaintl.com/products/k42)) + **K40 positioner** (socket contacts, $93.86 — [dmctools.com](https://dmctools.com/k40)). The AS79 79-way insert uses all size-22 solid barrel contacts (5A max, 38941-22 pin / 38943-22 socket). Confirmed: m-cal.com AS020-35SN "Primary Contacts Size: 22 AWG"; ecuplus.de AS620-35PN "79x 22 AWG". **The HDT-48-00 covers size 12/16/20 only — it does NOT support size-22 contacts and cannot be used for the AS79.**
  - **Maven HD30 35-pin accessories connector + DT 2-pin bypass connectors (size-16/20 contacts):** requires **Deutsch HDT-48-00** (~$350–465, [deutschconnector.com](https://www.deutschconnector.com/products/deutsch_connector_tools/deutsch_connector_crimp_tools/HDT-48-00/)) or budget JRready NEW-DT2 (~$169, Amazon `B09B562XDT`). These tools cover size 12/16/20 solid barrel contacts with no die swap. Cannot substitute a generic open-barrel die — wrong geometry produces cold crimps that pass pull-test but fail under vibration.
...
- [ ] 🔧 Crimp all AS79 engine-side contacts (Daniels AFM8 + **K42** positioner for size-22 pin contacts, 22–26 AWG) and insert into AS79 mating plug body **center-out** (pin 79 first, then R4 → R3 → R2 → R1) — verify seating click on every pin. Note: K43 is the size-20 positioner — do not use for size-22 contacts. Sources: `firewall-bulkhead.wv` crimp note; crimptech.com.au/autosport-tooling-guide.
...
- [ ] ⚠️ Build MaxxECU 07K engine harness offline (VW sensor connector ends, MaxxECU CMC on ECU side) — budget $300–500 in connector ends, wire, and conduit
  ...
  - > ⚠️ **Throttle wiring depends on path chosen above:**
    - **DBW (Option A):** Wire TB (Motor+, Motor−, TPS1, TPS2, 5V, GND) to MaxxECU e-throttle H-bridge pins (Motor+/− → C2 H4/H2 MOTOR 1+/1−, NOT GPO 3/4). The BMW E46 accelerator pedal is a separate 6-wire cabin run to the Maven HD30 Connector A cabin face (pins A14–A19) — NOT through the AS79 and NOT part of the 07K engine harness. APS does not cross to the engine side. Both pedal and TB use dual-track sensors; wire both tracks to separate MaxxECU analog inputs. ME7.1.1 pinout: DBW TB pins 84/92/117/118; APS1+2 pins 35/72 and 34/73. Source: `harnesses/firewall-bulkhead-dual.wv`.
...
- [ ] 🔧 Wire e-pedal (cabin-to-cabin) — run 6-wire shielded cable from pedal footwell → Maven HD30 Connector A cabin face pins A14–A19 → MaxxECU C2 APS1/APS2 + 5V refs + GNDs. No AS79 crossing. Source: `harnesses/firewall-bulkhead-dual.wv`.
