...
- [ ] ⚠️ Install column-assist EPS unit (under dash) — see `walkthroughs/13-eps.md`
  ...
  - > **Phase 3 provision:** At PMU16 installation (07K swap), move the EPS Controller IGN wire from the Phase 1 relay board rail → **PMU16 O8** (15A output, O6–O11 range; ~10A expected EPS load). One wire re-termination; no other EPS changes at Phase 3. Source: `harnesses/eps-column.wv` lines 58–63; `harnesses/power-distribution.wv` O8.
...
- [ ] ✅ Install DCT Shifter E36 unit ([dctshifter.com](https://dctshifter.com/)) — **3-wire digital connection to MaxxECU** — wiring: [`dct-shifter.wv`](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/dct-shifter.wv) ([diagram](https://htmlpreview.github.io/?https://github.com/wesleyxcooper/e36-wiring/blob/main/output/dct-shifter.html))
  - Models: 270GT Aluminum (compact), 290GTE Carbon (taller throw), 390GT (full-size) — all wire identically, choice is ergonomic
  - **Wiring (3 wires — passive momentary switches, no +12V supply required):** UP signal (Black) → MaxxECU DIN 2 = **Transmission UP shift**, DOWN signal (Blue) → MaxxECU DIN 1 = **Transmission DOWN shift**, GND (Brown) → MaxxECU Sensor GND. Source: [dctshifter.com/pages/installation §1](https://dctshifter.com/pages/installation) — *"The shifter uses a simple three-wire configuration with 2 digital switches inside… Black: UP · Blue: DOWN · Brown: 5V ground or chassis ground"*.
...
- [ ] 🔧 Install Radium Engineering 20-1170 fuel pump hanger + Walbro F90000267 (E85 from Phase 1)
  - **Purchase options (pick ONE — A recommended):**
    - **Option A (bundled — $193.19):** [JDT Racing F90000267 + Walbro 400-1168 install kit + JDT rewire kit](https://jdtracing.com/products/walbro-ti-f90000267-450lph-fuel-pump-w-install-kit-rewire-kit-e85-compatible) — Manufacturer PN `wal_F90000267_wal_400-1168_jdt_FPRK`. Includes pump, install kit, 16' 10 AWG power cable, 30A inline fuse, 3' 10 AWG ground, relay, relay pigtail, pass-through grommet, ring terminals, wire loom, cable ties. **One SKU, one shipping charge.**
    - **Option B (à la carte):** [Walbro F90000267 alone from Real Street Performance](https://www.realstreetperformance.com/walbro-universal-450lph-in-tank-fuel-pump-e85-version.html) ($181.12) + [JDT Universal Rewire Kit](https://jdtracing.com/products/jdt-racing-universal-fuel-pump-rewire-kit-w-10-gauge-power-ground-fuse-relay) ($37.99) = $219.11 across two vendors.
  - **Reference:** [Radium 20-1170 install instructions (PDF)](https://cdn.shopify.com/s/files/1/0887/4193/7449/files/19-0359.pdf) | [WireViz Phase 1](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/fuel-pump-hanger-phase1.wv) | [WireViz Phase 3](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/fuel-pump-hanger-phase3.wv) | [Circuit schematic](https://github.com/wesleyxcooper/e36-wiring/blob/main/schematics/fuel-pump-pwm.py)
  ...
  - > ⚠️ **Wiring — Phase 1 uses a discrete high-current relay (PMU16 is NOT installed until Phase 3):** MaxxECU GPO 2 (M50 pre-terminated harness 12-pin extra connector pin 3) → discrete 40/70A relay coil (JDT-supplied) → relay contact → 30A inline fuse → 10 AWG through cabin/tunnel → Radium hanger pump(+) stud. Pump(−) stud → dedicated chassis GND bolt (do NOT share with ECU sensor grounds). Vendor citation for the discrete-relay approach: [JDT Racing rewire kit description](https://jdtracing.com/products/walbro-ti-f90000267-450lph-fuel-pump-w-install-kit-rewire-kit-e85-compatible) — *"When installing a much larger than factory fuel pump you need to upgrade the wiring, fuse and relay setup."*
  - > ⚠️ **Phase 3 transition:** At the 07K swap, PMU16 arrives. Remove the Phase 1 discrete relay + 30A fuse (both from JDT kit). Land PMU16 O4 output on the SAME 10 AWG cabin/tunnel wire that previously came off the relay contact — no re-routing to the tank. The pump(+) stud connection at the hanger is unchanged. See `fuel-pump-hanger-phase3.wv`.
  - **MTune (Phase 1):** Outputs → Output config → GPO 2 → Function: **Fuel pump relay** (binary on/off; pump runs 100% duty when engine on). **MTune (Phase 3):** reassign to a CAN-broadcast function that PMU16 subscribes to → PWM fuel pump control, frequency 100–500 Hz, duty table 65% idle / 80% cruise / 100% WOT/boost.
...
- [ ] 🔧 Install Maven HD30 Dual 16+16 Bulkhead Kit — cabin-side receptacles + engine-side plugs into firewall plate   [ELECTRICAL] — wiring: [`firewall-crossing-maven.wv`](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/firewall-crossing-maven.wv)
  - > **Kit — $274 USD:** Maven Performance Products Dual Connector Bulkhead ([mavenspeed.com/collections/b2t-engineering/products/dual-connector-bulkhead](https://mavenspeed.com/collections/b2t-engineering/products/dual-connector-bulkhead), "16 PIN & 16 PIN"). Includes CNC billet aluminum plate (2.6" × 5.25", black anodized, pre-drilled + paper template) + 2× Deutsch HD30 shell-24 16-way connectors (both sides — flange receptacle + jam-nut plug per connector) + all size-16 solid contacts + stainless mounting fasteners.
  - > **Connector A (16-pin, cabin electronics):** CAN H/L/shield to Gauge.S cluster (3 pins) + DCT shifter paddle UP/DOWN/GND (3 pins). 8 spare cavities cavity-plugged.
  - > **Connector B (16-pin, safety-critical APS throttle):** Reserved exclusively for E46 accelerator pedal — ch1 SGND/+5V/sig + ch2 SGND/+5V/sig (6 pins). 10 spare cavities cavity-plugged. Any APS fault (open, short, sensor mismatch) triggers MaxxECU e-throttle shutdown. Isolated from Connector A to prevent accidental mixing with less-critical CAN + DCT signals.
  - > **Mount position — RHD build:** intake side of firewall (driver-opposite / passenger side) — same side as engine-bay MaxxECU RACE H2O and OEM harness grommet. Use rivnuts in the firewall for the plate mounting bolts. Use the paper template shipped with the kit to locate the plate.
  - > **NO high-current bypass needed:** Under the H2O engine-bay-mount arch, PMU16 is engine-bay-mounted alongside the MaxxECU. All high-current outputs (fan, EWP, AC relay coil) are engine-bay-to-engine-bay direct — no firewall crossing for power. See `harnesses/power-distribution.wv`.
  - > **Full wiring spec:** [`firewall-crossing-maven.wv`](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/firewall-crossing-maven.wv) · [`docs/wiring-bom.md` System 8](https://github.com/wesleyxcooper/e36-wiring/blob/main/docs/wiring-bom.md) · [`docs/dbw-pinouts.md`](https://github.com/wesleyxcooper/e36-wiring/blob/main/docs/dbw-pinouts.md) § Firewall Crossing Allocation

- [ ] 🔧 **Source the Deutsch HDT-48-00 crimper before starting the harness build**   [ELECTRICAL · TOOLING]
  - **Maven HD30 dual bulkhead + any DT 2-pin bypasses + PST-F1 pigtail (all Deutsch solid contacts):** requires **Deutsch HDT-48-00** — $197 (Maven-branded import equivalent, [mavenspeed.com](https://mavenspeed.com/collections/b2t-engineering/products/deutsch-crimp-tool-solid-contacts)) or $350–465 (genuine TE Connectivity, currently sold out at Maven; [deutschconnector.com](https://www.deutschconnector.com/products/deutsch_connector_tools/deutsch_connector_crimp_tools/HDT-48-00/)). One tool covers ALL Deutsch solid contacts across DT/DTM/DTP/DTHD/HD30 series per [deutschconnector.com selection guide](https://www.deutschconnector.com/technical/deutsch_connector_crimp_guide/) — no positioner set required.
  - **MaxxECU C1/C2 Molex CMC contacts (every 07K signal terminates here under H2O direct-terminate arch):** Molex 63811-9200 (small, 22-24 AWG) + 63811-8900 (large, 18-16 AWG). Every 07K harness wire terminates in these tools.
  - Also procure: Deutsch round-shoulder extraction tool (Maven or generic, $15-35) for HD30 depin; flush cutters (Milwaukee 48-22-6106, Home Depot `205652216`, ~$20, or upgrade to Knipex 78 91 125 Electronic Super Knips for precision work); rivnut tool (Astro Pneumatic 1442, Amazon `B003TODXQW`, ~$71) for firewall Maven plate mounting.
  - Tool budget total: ~$383 non-Molex — net savings of ~$611 vs the deprecated AS79 architecture that required Daniels AFM8 ($601.65) + K42 ($112.64) + K40 ($93.86) = $808.
...
- [ ] 🔧 **Mount and wire Ecumaster PMU16 power management unit**   [ELECTRICAL · POWER DISTRIBUTION]
  ...
  - **Post-shutdown EWP:** Program PMU16 to hold **O5+O14** active after key-off until CLT < 70°C CAN message from MaxxECU (or 3-min fallback timer) — no MaxxECU power-hold relay needed. Both outputs must be programmed; O14 is the second 25A leg of the parallel EWP supply.
...
- [ ] 🔧 Measure all wire lengths from MaxxECU CMC entry point (engine-bay ECU mount, intake-side firewall) to each component; add 10% slack; document against `.wv` file before cutting

### Phase B — Bench build  [ELECTRICAL]
- [ ] 🔧 Cut and label all main harness wires — PermaSleeve on both ends before crimping any terminal
- [ ] 🔧 Crimp all MaxxECU CMC (C1/C2) contacts (Molex 63811-9200 small / 63811-8900 large) and insert into C1 48-pin / C2 32-pin housings per `harnesses/maxxecu-07k.wv` — verify seating click on every pin. Direct-terminate at engine-bay ECU under H2O arch — no bulkhead in the engine-side signal path.
...
- [ ] ⚠️ **Bench continuity test — mandatory before sleeving:**
  ...
  - [ ] Shield drain terminates at CMC H1 (Sensor GND, pin 29) at the ECU end (single-end drain), not chassis GND
  - [ ] Pull-test: every MaxxECU CMC contact survives a firm hand tug
...
- [ ] 🔧 Apply main trunk PermaSleeve label (M21-500-C-342) at MaxxECU CMC entry: `ENGINE 07K PH3`
- [ ] ✅ Store completed harness with MaxxECU CMC plugs (C1 + C2) capped — install at Phase 3 swap day when the engine-bay MaxxECU RACE H2O is exposed for the M50 harness removal
...
- [ ] ✅ Order S&P titanium exhaust stud kit — **factory 07K studs are too short for any aftermarket manifold with a thicker flange**   [AIR · EXHAUST]
  - [S&P Automotive Titanium m8x1.25 exhaust studs](https://s-pautomotive.com/product/titanium-m8x1-25-exhaust-studs/) — **$9.05 ea × 15 = $135.75**. Titanium stud + Ti nut + Ti washer per unit. Won't seize or corrode under sustained heat cycles; higher tensile than steel; doesn't gall like stainless. Total length 40mm (13mm head-side thread + 8mm shoulder + 19mm nut-side thread). **4mm internal hex at head-side tip for installation** — thread by hand, snug with Allen key, then torque the nut. Standard install method on all quality VW/Audi stud kits (S&P, ARP, Arcane, 034Motorsport all use the broached-tip design). Order Phase 2, install at manifold fitment during Phase 3.
  - > **Why S&P over LPS/ARP:** LPS Ti stud kit is discontinued (lpsfab.com returns 404 as of Aug 2026). ARP stainless equivalent (Pro-Race UK, £187 ≈ $236 shipped) costs more, is stainless not Ti, ships from UK. Arcane Motorsport Ti kit (Australia, $199 AUD) ships from AU with international shipping. S&P Ti at $135.75 US-shipped is cheapest AND titanium AND same vendor as your engine stand adapter plate.
  - > **Ti vs ARP stainless for this application:** Ti benefits are weight (~40% lighter, negligible here), reusability (Ti won't seize into head — modest advantage over ARP), non-galling with SS nuts, corrosion immunity. ARP stainless is functionally comparable for a non-show build. Ti wins at this specific price point because S&P priced their kit below the ARP equivalent.
...
**In-Garage Lifting**
- **Hoist:** [HF 2-Ton Foldable Shop Crane, SKU 58755](https://www.harborfreight.com/2-ton-capacity-asme-pase-certified-foldable-shop-crane-58755.html) — **PURCHASED** ($369.99)
- **Stand:** [HF 1000 lb Engine Stand, SKU 59201](https://www.harborfreight.com/1000-lb-capacity-engine-stand-59201.html) — **PURCHASED** ($109.99) + [S&P Automotive 07K Engine Stand Mount](https://s-pautomotive.com/product/07k-engine-stand-mount/) — **PURCHASED** ($99). Attaches to dedicated **side-mount bosses on the exhaust side** of the 07K block (same side as OEM transverse engine mount). In RHD E36 longitudinal orientation (timing chain forward), exhaust side = driver side (right). Fits 59201 (2" ID Pittsburgh/HF head tube). No load leveler needed for pallet→stand.
...
- [ ] ✅ Install iABED longitudinal oil filter housing
  - iABED housing has a dedicated **turbo oil feed port** — install **-4AN fitting** (M12×1.5 to -4AN adapter or direct -4AN, per iABED port spec) at this port for the G25-660 RR oil feed line. Confirmed by @wingman703. Include a **1.0 mm (0.040") orifice restrictor** inline on the feed line — required for the G25-660's dual ball bearing CHRA ([Full Race](https://www.full-race.com/garrett-g-series-g25-660-turbocharger)), targets 40 psi at turbo inlet at max engine speed per [Garrett](https://www.garrettmotion.com/knowledge-center-category/racing-and-performance/turbo-oil-restrictors-oil-supply/). Journal-bearing guidance does NOT apply — do not remove the restrictor. **The G25-660 ships with a -4AN + 1.0 mm restrictor fitting in the box** per Full Race — verify at receipt before sourcing separately.
...
- [ ] ⚠️ **Send 07K oil pump to Linder Power Systems (LPS) for oil pump mod** (send-in service — no storefront)
  - Contact LPS: `linderpowersystems@gmail.com`, IG [@linder_power_systems_lpsfab](https://www.instagram.com/linder_power_systems_lpsfab/), or [lpsfab.com](https://lpsfab.com)
  - **Pricing per [LPS YouTube video description](https://www.youtube.com/watch?v=nXsUm8ph2Ys):** $150 for the mod on your pump; $420 turnkey (LPS supplies new pump + mod, availability-dependent).
  - **Mod scope per Justin Linder** ([LPS YouTube transcript](https://www.youtube.com/watch?v=nXsUm8ph2Ys)): user-adjustable pressure relief valve, factory-set to 80–85 psi cold with SAE30. Verbatim: *"I came up with a way to modify the oil pump so it's user adjustable — we're going to factory set these at between 80 and 85 PSI. The factory pump only puts out between 60 and 65 PSI on a cold engine and lower as it gets [warm]."*
  - **PN compatibility per LPS video description**: *"This mod is only available for the early style oil pump ... If supplied by us you will get the latest version oil pump (currently `07k115105Q`) modified."* `07K115105Q` is LPS-confirmed. F/G/J/P/R/T/AC/AK: DM LPS to confirm.
  - **Verify by casting stamp** on the pump body — cast shows PN + `AlSi9Cu3` alloy spec + `MEXICO` (Puebla plant). Photograph and DM LPS before purchasing/shipping.
  - **LPS does not modify** the drive sprocket (external, single-use 60Nm+90° stretch bolt) or pickup tube (external, O-ring). Only the pump body needs shipping; sprocket attached is preferred but not required; pickup tube not needed (reuse existing motor's with new O-ring).
  - **Sourcing (ranked):** (1) local donor pump alongside forged crank, $30–100; (2) [eBay 187826468192](https://www.ebay.com/itm/187826468192) $34 used OEM `G` (casting-stamp verified, includes sprocket); (3) eBay literal `07K115105Q` $30–90 used; (4) [VWPartsGiant `G`](https://www.vwpartsgiant.com/genuine/volkswagen~oil-pump~07k115105g) $246.84 or [Europa Parts `Q`](https://www.europaparts.com/oil-pump-07k115105q.html) $249.95 new OEM; (5) LPS turnkey $420. OEM only — do not buy sub-$50 aftermarket.
  - Mandatory for drifting/9k RPM use — stock 60–65 psi cold is inadequate for boost + upgraded valvetrain (VVT starves).
  - Full detail: <ref_file file="/Users/wesleyc/personal/e36/e36-docs/walkthroughs/24-07k-oil-system.md" /> § 1 + § 1a
  - > ⚠️ **Pitfall:** LPS lead time can be 4–8 weeks. Send early in Phase 2 so it's back before assembly is complete.
...
- [ ] ⚠️ Build MaxxECU 07K engine harness offline (VW sensor connector ends, MaxxECU CMC on ECU side) — budget $300–500 in connector ends, wire, and conduit
  ...
  - > ⚠️ **Throttle wiring depends on path chosen above:**
    - **DBW (Option A):** Wire TB (Motor+, Motor−, TPS1, TPS2, 5V, GND) direct-terminate at engine-bay MaxxECU C1/C2 (Motor+/− → C2 H4/H2 MOTOR 1+/1−, NOT GPO 3/4; TPS1 → C1 G2, TPS2 → C1 J2 AIN 2). TB is engine-bay-to-engine-bay — no bulkhead in the TB path. The BMW E46 accelerator pedal is a separate 6-wire cabin run to the Maven HD30 **Connector B (safety-critical)** cabin face pins 1-6 → engine-side plug → MaxxECU. Both pedal and TB use dual-track sensors; wire both tracks to separate MaxxECU analog inputs. ME7.1.1 pinout: DBW TB pins 84/92/117/118; APS1+2 pins 35/72 and 34/73. Source: `harnesses/firewall-crossing-maven.wv`, `docs/dbw-pinouts.md`.
...
- [ ] 🔧 Wire e-pedal — run 6-wire shielded cable from pedal footwell → Maven HD30 **Connector B (safety-critical)** cabin face pins 1-6 → engine-side Maven B plug → engine-bay MaxxECU C2 (APS1 → C2 E4 AIN 6, APS2 → C2 F1 AIN 7) + C1 G1 (+5V rails) + C1 H1 (SGND rails). Connector B isolated from Connector A (CAN + DCT) for safety-critical throttle input. Source: `harnesses/firewall-crossing-maven.wv`, `harnesses/epedal-bmw-e46.wv`, `docs/dbw-pinouts.md` § Firewall Crossing Allocation.
