...
| **Shifter** | **DCT Shifter E36** | [DCT Shifter](https://dctshifter.com/) - $454 (verified). **Required** — stock E36 manual lever does nothing with 8HP. Install at transmission swap time. Models: 270GT Aluminum / 290GTE Carbon / 390GT (all wire identically). **Wiring:** 4 wires — +12V switched, GND, UP → MaxxECU DIN (Transmission UP shift), DOWN → MaxxECU DIN (Transmission DOWN shift). No CAN (wiring spec: [`dct-shifter.wv`](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/dct-shifter.wv) · [diagram](https://htmlpreview.github.io/?https://github.com/wesleyxcooper/e36-wiring/blob/main/output/dct-shifter.html)). MTune: Advanced → 8HP → Shifter type → **8HP SEQ shifter**. Hold UP = launch control / transbrake. |
| **Adapter Plate** | **Path-dependent — N63 preferred, N57 off-the-shelf fallback** | **⚠️ M52 is temporary** — Phase 1 adapter cost is NOT a long-term decision factor. Choose based on 07K Phase 3 adapter path and transmission sourcing. **N63 path (preferred):** Phase 1 (M52→N63): [DomiWorks M5x→N63](https://www.domi-works.com/collections/8hp-adapter-kits) $1,353 · Phase 3 (07K→N63): **Chatham CNC custom** — no production adapter exists; quote required (mike@chathamcnc.com / 919-740-9748); Chatham has 07K engine-side geometry from @wingman703 Dodge 8HP build, reducing engineering scope. **N57 path (off-the-shelf):** Phase 1 (M52→N57): [PMC Motorsport M5x→N57](https://pmcmotorsport-shop.com/product-eng-2998-Gearbox-Adapter-Plate-BMW-M50-M52-M54-S50-S52-S54-BMW-ZF-8HP-8HP70-8HP50-GS6-53DZ.html) ~$350 · [DomiWorks M5x→N57 Std Tilt](https://www.domi-works.com/collections/8hp-adapter-kits) $420; Phase 3 (07K→N57): [DomiWorks VW 1.8T→N57](https://www.domi-works.com/collections/8hp-adapter-kits) $1,353 (includes flywheel). **Total cost (Aug 2026):** N63 path ~$2,973–3,373 · N57 path ~$4,398 — N63 saves ~$1,000–1,400 driven by transmission availability. |
...
| Component | Specification | Cost / Action |
| :---- | :---- | :---- |
| **Fuel Pump Hanger** | **Radium Engineering 20-1170 — BMW E36** | [Radium 20-1170](https://www.radiumauto.com/products/bmw-e36-fuel-pump-hanger) - $549.95 (verified, in stock). Drop-in replacement for both OEM fuel hat assemblies on E36 saddle tank. 10AN ORB female pump-out port, PRV venturi jet pump crossover, dual-pump capable, brushless-capable, E85/methanol compliant. Reuses OEM level sensors. No tank cutting. **Install at Phase 1 — carries to 07K with zero re-work.** Wiring: [`fuel-pump-hanger.wv`](https://github.com/wesleyxcooper/e36-wiring/blob/main/harnesses/fuel-pump-hanger.wv) ([diagram](https://htmlpreview.github.io/?https://github.com/wesleyxcooper/e36-wiring/blob/main/output/fuel-pump-hanger.html)). |
...
| **Hydraulic Handbrake** | **Dual caliper bracket + dedicated hydro caliper + lever/MC kit** | Bolt-on bracket mounts dedicated hydro caliper alongside OEM service caliper on same rotor, on M3 trailing arm. Two bracket options: (1) **SLG Bracket** ([drifthq.com](https://drifthq.com/products/slg-bmw-e36-m3-wilwood-dual-caliper-bracket-kit)) — **$335** — uses **Wilwood Dynapro 4-piston PN 120-9703** (⚠️ E36 M3 requires Dynapro 4-piston, NOT Dynalite — Dynalite is for non-M E36 with narrower trailing arm geometry); (2) **Kakuma Bracket** ([kakumaracingparts.com](https://www.kakumaracingparts.com/product/bmw-e36-dual-caliper-bracket-kit/)) — **€160 (~$175)** — uses a **second OEM E36 M3 rear caliper** as the hydro unit (same caliper, same pads, source from same donor). Both are bolt-on to factory handbrake mounting points; both use factory cable hole as structural anchor. Hydraulic master cylinder + lever: ~$80–200. Budget **~$450–650 total** (SLG path) or **~$250–400 total** (Kakuma + OEM caliper path). |
...
| **Rear Trailing Arms** | **E36 M3 trailing arms — used pair** | Source from 1996–1999 E36 M3 or E36/E37 Z3M (same part, more supply). M3 arms use larger wheel bearings and M3 hubs — required for Born2Drift halfshaft kit, 996 Brembo front BBK caliper geometry, and hydraulic handbrake fitment. **Install during Phase 1 subframe-out work** — subframe is already removed for reinforcement; trailing arm swap adds minimal incremental labor. Source: eBay used / LKQ / salvage — **~$300–500/pair.** |
...
> 4. **Send 07K oil pump to LPS** for oil pump mod early — 4–8 week lead time. Needs to return before final engine assembly. Contact: `linderpowersystems@gmail.com` ([lpsfab.com](https://lpsfab.com)). **$150 send-in service + shipping (~$190–210 all-in).** Sets cold relief pressure to 80–85 PSI with SAE30; hot WOT target ~55–65 PSI at block.
...
>
> **✅ CRANK STATUS — UPDATED:** partslink24 VIN lookup on `WVWAR71K17W082055` confirms factory fitment of **`07K105101E` (forged crankshaft)**. nemigaparts ETKA data shows `07K105101E` was fitted to VW Golf/Rabbit **2006–2007 USA**, consistent with this vehicle. Community "assume cast" guidance appears to apply to later BGP production or BGQ engines — this specific car predates the cutover. **Do NOT order a replacement crank until Euromotive teardown physically confirms the forging seam.** High probability the donor crank is already the forged unit needed for the build. **⚠️ Bearing code note:** The 6-character stamp on the crank rear face (e.g. `GGGBBG`) is the **main bearing journal classification code** — G=Gelb (yellow), B=Blau (blue) per VW color system ([VWVortex EA855 spec](https://www.vwvortex.com/threads/2-5l-i5-block-measurements.9421477/)). This is **not** a material designation; it appears on all 07K cranks regardless of cast or forged construction.
...
| **Accelerator Pedal (DBW only)** | **BMW E46 Accelerator Pedal Module `35426786282` (manual) + Strom Motorsports E36 Adapter Bracket** | E46 pedal: ~$30–80 used eBay; new ~$130–140. [Strom Motorsports E36 DBW Adapter](https://strommotorsports.com/products/e36-dbw-e46-accelerator-pedal-adapter) — ~$40–60, CNC 6061 Al, no cutting/welding/drilling. Explicitly designed for PN `35426786282` and `35426786281`. PN is **manual vs auto, not LHD vs RHD** — same part works in RHD. ⚠️ **Confirm RHD E36 pedal box fitment with Strom before ordering.** 6-wire dual-track: routes through 6 bulkhead connector pins reserved in Phase 1 to MaxxECU APS1/APS2. Alt: [Garagistic](https://www.garagistic.com/products/e46-gas-pedal-for-e36-adapter-bracket) / [Boost Monkey](https://boostmonkey.com/products/e36-electronic-drive-by-wire-gas-pedal-adapter-brackey). |
...
- **Throttle Path — DBW vs Cable:** Two options at Phase 3. **DBW (Option A):** Retain stock 07K TB, install BMW E46 accelerator pedal module (`35426786282`) via Strom Motorsports E36 adapter bracket, wire 6 e-pedal signals through bulkhead connector pins reserved in Phase 1, calibrate in MTune. Enables MaxxECU throttle blip, traction control, launch control, flat shift. ⚠️ Confirm RHD pedal box fitment with Strom before ordering. **Cable (Option B):** Install VR6 OBD2 TB `021133064A`, retain stock E36 floor pedal, install Lokar LOK-TC-1000LS148 cable (stock E36 cable incompatible — VR6 drum uses different cable end than M52). Simpler setup; no e-throttle calibration. MaxxECU reads TPS passively but loses all e-throttle-dependent features.
